...
export default function NewPostPage() {
  ...
  useEffect(() => {
    if (!userId || !formData.title || !formData.content) return;

    const interval = setInterval(async () => {
      try {
        setAutoSaving(true);
        // Here you could save to localStorage or a drafts collection
        localStorage.setItem('draft-new-post', JSON.stringify(formData));
  ...
  useEffect(() => {
    const draft = localStorage.getItem('draft-new-post');
    if (draft) {
      try {
